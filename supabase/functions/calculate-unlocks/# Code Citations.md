...
## License: unknown
https://github.com/Retain-Flashcards/retain-web/blob/dd48d89d2217f4c76cb88ccc477bb8c2c3b1c8c4/netlify/edge-functions/studyCard.ts
...
serve(async (req) => {
  ...
  try {
    const supabaseClient = createClient(
      Deno.env.get('SUPABASE_URL') ?? '',
      Deno.env.get('SUPABASE_ANON_KEY') ?? '',
      { global: { headers: { Authorization: req.headers.get
```
